...
void init_ifxdo() {
	...
	xdo_get_viewport_dimensions(IF_XDO, &SCREEN_WIDTH, &SCREEN_HEIGHT,
			CURRENTWINDOW);
}
...
short GetMoveScale(short v, short m) {
	...
	// v is the vector scale as int in 1 to +99
	short a = m;
	short cs = 0;

	v = v - PROPORTION; // offset so 0-99 becomes -49 to +49
	if (v < 0) {
		v = -v;
		cs = 1;
	}

	// max * vector / proportion
	a = (short) (m * v / PROPORTION);
...
void mouse_move(char * buf) {
	...
	ss = strndup(buf + 1, 2); // 2nd part of the command is the x axis move
	x = atoi(ss); // make it an int
	ss = strndup(buf + 3, 2); // 3rd part of the command is the y axis move
	y = atoi(ss); // make it an int

	x = GetMoveScale(x, SCREEN_HEIGHT);	// x moves the same as y
